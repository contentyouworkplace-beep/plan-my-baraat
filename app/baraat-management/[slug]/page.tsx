...
export function generateStaticParams() {
  const locationSlugs = ALL_BARAAT_LOCATIONS.map((loc) => ({ slug: loc.slug }));
  const seoKeywordSlugs = SEO_KEYWORD_PAGES.map((page) => ({ slug: page.slug }));
...
export async function generateMetadata({
  ...
  const content = getSeoPageContent(params.slug);
  if (!content) return {};
  ...
  return {
    ...
    alternates: {
      canonical: `/baraat-management/${params.slug}`,
    },
    openGraph: {
      title: content.metaTitle,
      description: content.metaDescription,
      url: `/baraat-management/${params.slug}`,
...
export default function BaraatManagementPage({
  params,
}: {
  params: { slug: string };
}) {
  ...
  const breadcrumbItems = [
    ...
    { name: "Baraat Management", url: `${BASE_URL}/baraat-management` },
    ...(loc?.parentCity ? [{ name: loc.parentCity.toUpperCase(), url: `${BASE_URL}/baraat-management/${loc.parentCity}` }] : []),
    { name: loc ? loc.name : content.pageTitle, url: `${BASE_URL}/baraat-management/${params.slug}` },
  ];
  ...
  const jsonLdService = generateJsonLdServiceGeneric({
    ...
    url: `${BASE_URL}/baraat-management/${params.slug}`,
  });
  ...
  if (loc) {
    ...
    relatedLinks = priorityCities.map((c) => ({
      ...
      href: `/baraat-management/${params.slug}-${c.slug}`,
    }));
  }
